...
const JWT_SECRET = process.env.JWT_SECRET || (process.env.NODE_ENV === 'production' ? null : 'dev-only-sslt10-secret-change-me');
...
async function authenticate(username,password){
  ...
  if(!clean||!password)throw new HostAuthError('Username and password required');
  ...
  if(!user||!(await bcrypt.compare(String(password),user.password_hash)))throw new HostAuthError('Invalid username or password.');
...
async function setUserPassword(userId,nextPassword){if(!nextPassword||String(nextPassword).length<4)throw new HostAuthError('Password must be at least 4 characters');const hash=await bcrypt.hash(String(nextPassword),SALT_ROUNDS);db.run("UPDATE users SET password_hash=?,updated_at=datetime('now') WHERE id=?",[hash,userId]);}
