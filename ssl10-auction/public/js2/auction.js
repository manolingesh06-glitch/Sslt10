...
(async function init(){
  ...
  liveDocRef.onSnapshot((snap) => {
    const d = snap.data() || {};
    // render() rebuilds the team strip, lot card, and owner controls —
    // real work, not free. Every single live write (including a single
    // chat typing ping) fires this callback, so with 10+ people some of
    // whom are typing during active bidding, calling render() unconditionally
    // here means constant unnecessary DOM rebuilds. Only call it when a
    // field it actually depends on changed.
...
function render(){
  ...
  if (currentIdx !== lastRenderedIdx){
    ...
    gsap.fromTo('#lotCard', { opacity: 0.4, y: 8 }, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' });
  }
...
function closePanel(panel, sheet){
  ...
  gsap.to(panel, { opacity: 0, duration: 0.25, delay: 0.05,
