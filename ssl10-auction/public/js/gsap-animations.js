...
(function(){
  ...
  Anim.flash = function(el, color){
    ...
    gsap.fromTo(el,
      {backgroundColor: color || 'rgba(255,176,32,0.28)'},
      {backgroundColor: prevBg || 'rgba(255,176,32,0)', duration:0.9, ease:'power2.out',
  ...
  function initWatchers(){
    ...
    watchScreen('setupScreen');
    watchScreen('setupDashboard', {cardSelector: '.admin-section, .card'});
    watchScreen('setupKeyBox');
  }
  ...
  // NOTE: the whole-app entrance (mainApp becoming visible after login) is
  // intentionally NOT wrapped here. watchScreen('mainApp', ...) below already
  // observes that exact same classList change and fires Anim.pageIn for it.
  // Wrapping enterApp() too caused pageIn to fire twice per login (confirmed
  // via test harness) — one call from this wrap, one from the observer's
  // async MutationObserver callback — producing a visible double fade/stagger.
  // animateInitialScreen() (below) still separately covers the session-restore
  // case where mainApp is already visible at page load with no class change.
